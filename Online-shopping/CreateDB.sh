...
INSERT INTO `categories` VALUES (1,'Tv'),(2,'Phone'),(3,'Notebook'),(4,'Photo'),(5,'Fridge'),(6,'Tabled'),(7,'Other');
...
CREATE TABLE `orders` (
  ...
  KEY `FK_e339pox2sojm3is3iyb5ue2rn` (`user_id_FK`),
  CONSTRAINT `FK_e339pox2sojm3is3iyb5ue2rn` FOREIGN KEY (`user_id_FK`) REFERENCES `users` (`user_id`)
...
CREATE TABLE `ordered_products` (
  ...
  KEY `FK_46wg99is9vxso39tv2r11gviv` (`product_id_FK`),
  KEY `FK_iapi8aya16t6sfkuuhexlopkv` (`order_id_FK`),
  CONSTRAINT `FK_46wg99is9vxso39tv2r11gviv` FOREIGN KEY (`product_id_FK`) REFERENCES `products` (`product_id`),
  CONSTRAINT `FK_iapi8aya16t6sfkuuhexlopkv` FOREIGN KEY (`order_id_FK`) REFERENCES `orders` (`order_id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8;

INSERT INTO `ordered_products` VALUES (1,2,3,4),(2,3,3,5),(3,3,4,5),(4,2,2,5);