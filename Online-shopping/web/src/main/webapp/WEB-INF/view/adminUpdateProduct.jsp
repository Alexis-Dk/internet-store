...
<%-- 	  		            <sec:authorize access="hasRole('admin')">
			           	...
			           	<sec:authorize access="hasRole('admin')">
		                    <c:forEach items="${requestScope.productCategory}" var="category">
				            	<li><a href="productAdmin?category=<c:out value="${category.categoryId}"></c:out>"><c:out value="${category.categoryName}"> </c:out></a></li>
				            </c:forEach>
			           	</sec:authorize>
<%-- 	  				<sec:authorize access="isAnonymous() or hasRole('user')"> --%>			           		
                        ...
                        <sec:authorize access="hasRole('admin')">
                        <li class="active"><a>Update product</a></li>
                        </sec:authorize>
                    </ul>
                </div>  
            </div>
		...
		<div id="addProductForm">
			...
			<sf:form class="form-horizontal" name="productDTO"
				...
				<div class="form-group" <%= request.getAttribute("categoryCharacteristicEnableStrStatus1") %>>
					<div class="col-xs-3">
						<label for="productNameData" class="control-label"><%= request.getAttribute("categoryCharacteristicStr1") %>:</label>
					</div>
					<div class="col-xs-5">
